...
const SYSTEM_PROMPT_TITLE = 'Genera un título corto (máximo 8 palabras) que resuma el tema del siguiente contenido académico. Responde únicamente con el título, sin comillas ni puntuación final.';

async function generateShortTitle(content: string, model: string): Promise<string> {
  try {
    const result = await generateFromAI('nineRouter', SYSTEM_PROMPT_TITLE, content, null, {
      model,
      temperature: 0.3,
      max_tokens: 30,
    });
    ...
    logger.warn('Error generando título de bloque de conocimiento, se usa fallback', { error: (err as Error).message });
...
export async function extractBlocks(
  ...
  for (const seg of verificable) {
    if (alreadyExtracted.has(seg.messageId)) continue;
    ...
    const title = await generateShortTitle(msg.content, model);
